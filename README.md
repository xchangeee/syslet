...
syslet reads JSON specs from stdin, a directory, or a single JSON-stream file, validates them, and applies the desired state:
...
JSON specs can be updated directly over SSH by piping them into `syslet --stdin` (see [Deploy](#4-deploy)).
...
Units already marked with `"removalAllowed": true` and not present in the current input will be pruned from the host.
...
Requires Go 1.25+. This produces the server-side binary that applies specs:

- `build/syslet-<arch>` — runs on the host, reads specs from stdin, a file, or a directory
...
Pipe the spec files straight into `syslet --stdin` over SSH — no extra tooling, one round-trip:
...
`syslet --stdin` accepts a JSON array, newline-delimited JSON, or concatenated JSON objects
(exactly what `cat dir/*.json` produces). On apply it persists the received stream to
`/etc/syslet/config.json` as the host's on-disk record, so it can be re-applied manually:
...
#### push a directory manually

`syslet` can also read a directory of `.json` spec files directly (useful for git repositories):
...
```
/etc/syslet/
└── config.json                     # spec stream persisted on apply (re-applyable record)
