...
State is stored in SQLite at `/var/syslet/state.db`. Override with `SYSLET_DB=/path/to/state.db`.
...
syslet stores all specs in SQLite and generates/overwrites quadlet files in `/etc/containers/systemd/` and config files in `/etc/containers/config/` on every reconciliation cycle.
