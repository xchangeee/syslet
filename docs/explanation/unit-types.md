...
`volume`, `network`, and `build` share a `reclaimPolicy` field controlling whether their backing podman resource is deleted or retained when the unit is removed (see [Safety mechanisms](safety-mechanisms.md)); `container` does not have one, since removing a container's unit always stops and removes the container itself.
