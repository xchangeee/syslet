...
syslet keeps the useful part of that model, a declarative desired state the host converges to, without running a loop of its own.
The parts that need continuous supervision are left to systemd:
...
The tradeoff is explicit: syslet does not detect or correct drift introduced between invocations by something other than syslet itself (e.g. someone manually editing a unit file, or Podman state diverging in a way not deducible from the previous apply).
Re-running syslet re-establishes the desired state, but nothing does that for you automatically.
You decide when reconciliation happens.

This is acceptable because syslet assumes it's the sole manager of the host, and that when several people work on it, every change goes through a git repository rather than by hand.
Drift is then the exception rather than something to guard against continuously.
