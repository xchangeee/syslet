...
- **Kubernetes / k3s / k0s** — level-triggered, continuously reconciling; brings a scheduler, API server, and cluster networking model that a single-node deployment doesn't need. syslet targets the case where that overhead isn't justified. See [Why syslet](../index.md#why-syslet) on the homepage.
